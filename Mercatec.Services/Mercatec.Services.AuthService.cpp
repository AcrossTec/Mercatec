...
namespace Mercatec::Services
{
    ...
    AuthService& AuthService::GetInstance() noexcept
    {
    ...
    IObservableVector<AuthService::AccountStore_t::Account> AuthService::GetUserAccountsForDevice(const Guid& device_id) const noexcept
    {
        return m_AccountStore.GetUserAccountsForDevice(device_id);
    }

    void AuthService::Register(const std::wstring_view user_name) noexcept
    {
        m_AccountStore.AddAccount(user_name);
    }
    ...
    bool AuthService::PassportRemoveDevice(const Guid& user_id, Guid device_id) noexcept
    {
    ...
    bool AuthService::ValidateCredentials(const std::wstring_view user_name, const std::wstring_view password)
    {
        if ( not user_name.empty() and not password.empty() )
        {
            ...
            Guid user_id = GetUserId(user_name);

            if ( user_id != GuidHelper::Empty() )
            {
                AccountStore_t::Account account = GetUserAccount(user_id);
    ...
    bool AuthService::SendServerSignedChallenge(const Guid& user_id, const Guid& device_id, [[maybe_unused]] const array_view<Byte> signed_challenge)
    {
