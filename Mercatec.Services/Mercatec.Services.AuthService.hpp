...
namespace Mercatec::Services
{
    ...
    public:
        ...
        static AuthService& Instance() noexcept;
        ...
        IObservableVector<AccountStore_t::Account> GetUserAccountsForDevice(const std::wstring_view device_id) const noexcept;

        void Register(const std::wstring_view user_name, const std::wstring_view password) noexcept;
        bool PassportRemoveUser(const Guid& user_id) noexcept;
        bool PassportRemoveDevice(const Guid& user_id, const std::wstring_view device_id) noexcept;
        ...
        bool SendServerSignedChallenge(const Guid& user_id, const std::wstring_view device_id, const array_view<Byte> signed_challenge);
