#include "pch.h"
#include "Mercatec.WinUIEx.Internal.Icon.hpp"
...
namespace Mercatec::WinUIEx::Internal
{
    ...
    static HANDLE InternalLoadImage(const std::wstring_view FileName) noexcept
    {
        ...
        return ::LoadImageW(nullptr, FileName.data(), EnumType::GetValue(ImageIcon), 16, 16, EnumFlags::GetValue(LrLoadFromFile));
    }

    static HICON InternalLoadImage(const winrt::array_view<const std::byte> Rgba, const winrt::array_view<const std::byte>::size_type Size)
    {
        ...
        Icon::ThrowIfInvalid(Handle);
        return Handle;
    }

    static HICON InternalYang() noexcept
    {
        BYTE AndMaskIcon[] =        //
        ...
        HMODULE HInstance = ::GetModuleHandleW(nullptr);
        return ::CreateIcon(HInstance, 32, 32, 1, 1, AndMaskIcon, XOrMaskIcon);
    }

    std::unique_ptr<Icon> Icon::MakeYang() noexcept
    {
        return std::make_unique<Icon>(InternalYang());
    }

    std::unique_ptr<Icon> Icon::MakeFromFile(const std::wstring_view FileName)
    {
        return std::make_unique<Icon>(HICON(InternalLoadImage(FileName)));
    }

    std::unique_ptr<Icon> Icon::MakeFromByteArray(const winrt::array_view<const std::byte> Rgba, const winrt::array_view<const std::byte>::size_type Size)
    {
        return std::make_unique<Icon>(InternalLoadImage(Rgba, Size));
    }

    Icon Icon::FromFile(const std::wstring_view FileName)
    {
        return Icon(HICON(InternalLoadImage(FileName)));
    }

    Icon Icon::FromByteArray(const winrt::array_view<const std::byte> Rgba, const winrt::array_view<const std::byte>::size_type Size)
    {
        return Icon(InternalLoadImage(Rgba, Size));
    }
    ...
    /// </returns>
    Icon Icon::Yang() noexcept
    {
        return Icon(InternalYang());
    }
} // namespace Mercatec::WinUIEx::Internal
